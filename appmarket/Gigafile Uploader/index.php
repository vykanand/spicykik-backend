...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
  ...
  <script>
    // Filebin.net API endpoint
    const FILEBIN_API = 'https://filebin.net';
    
    // Optional: You can get an API key from Filebin for higher limits
    // const API_KEY = 'your-filebin-api-key';
    ...
    async function uploadFiles() {
      ...
      try {
        // Create a bin for all files
        const binResponse = await fetch(`${FILEBIN_API}/api/bin`, {
          method: 'POST',
          headers: {
        ...
        if (!binResponse.ok) {
          throw new Error('Failed to create file bin');
        }

        const binData = await binResponse.json();
        const binId = binData.bin;
        const binUrl = `${FILEBIN_FILE}/${binId}`;

        // Upload each file to the created bin
        for (const file of files) {
          const formData = new FormData();
          formData.append('file', file);

          const uploadResponse = await fetch(`${FILEBIN_API}/api/bin/${binId}`, {
            method: 'POST',
            body: formData,
            headers: {
              // Uncomment and add your API key if you have one
          ...
          const fileUrl = `${binUrl}/${file.name}`;
          ...
          if (window.parent !== window) {
            window.parent.postMessage({ type: 'fileUploaded', url: fileUrl, fileName: file.name }, '*');
          }
        }

        // Add bin URL at the top
        const binContainer = document.createElement('div');
        binContainer.className = 'bg-blue-50 border-l-4 border-blue-400 p-4 mb-4 rounded';
        binContainer.innerHTML = `
          <p class="font-medium text-blue-700">All files uploaded successfully!</p>
          <p class="text-sm text-blue-600 mt-1">Bin URL: <a href="${binUrl}" target="_blank" class="underline">${binUrl}</a></p>
        `;
        statusDiv.insertBefore(binContainer, statusDiv.firstChild);
      } catch (error) {
