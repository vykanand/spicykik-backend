...
(function make_xlsx(XLSX) {
  ...
  var make_ssf = function make_ssf(SSF) {
        ...
        case 115 /* 's' seconds */:
          ...
          switch (fmt) {
            ...
            case ".000":
              ...
              if (ss >= 60 * tt) ss = 0;
              if (fmt === "s") return ss === 0 ? "0" : String(ss / tt);
              o = pad0(ss, 2 + ss0);
    ...
    var write_num = (function make_write_num() {
      ...
      function rnd(val, d) {
        ...
        return String(Math.round(val * dd) / dd);
      }
      function dec(val, d) {
      ...
      function flr(val) {
        if (val < 2147483647 && val > -2147483648)
          return String(val >= 0 ? val | 0 : (val - 1) | 0);
      ...
      function write_num_flt(type, fmt, val) {
        ...
        if ((r = fmt.match(/^#,##0\.([#0]*0)$/)) !== null) {
          return val < 0
            ...
            : commaify(String(Math.floor(val))) +
                "." +
      ...
      function write_num_int(type, fmt, val) {
        ...
        if ((r = fmt.match(/^#,##0(\.?)$/)) !== null)
          return sign + commaify(String(aval));
        if ((r = fmt.match(/^#,##0\.([#0]*0)$/)) !== null) {
          return val < 0
            ? "-" + write_num_int(type, fmt, -val)
            : commaify(String(val)) + "." + fill("0", r[1].length);
        }
        if ((r = fmt.match(/^#,#*,#0/)) !== null)
  ...
  function parse_ws_xml(data, opts, rels) {
    ...
    if (opts.sheetRows > 0 && s["!ref"]) {
      ...
      if (opts.sheetRows < Number(tmpref.e.r)) {
  ...
  function parse_ws_xml_cols(columns, cols) {
    ...
    for (var coli = 0; coli != cols.length; ++coli) {
      ...
      if (!seencol && coll.width) {
        seencol = true;
        find_mdw(Number(coll.width), coll);
      }
      if (coll.width) {
        coll.wpx = width2px(Number(coll.width));
  ...
  function write_ws_xml_cell(cell, ref, ws, opts, idx, wb) {
    ...
    switch (cell.t) {
      ...
      case "d":
        if (opts.cellDates) vv = new Date(cell.v).toISOString();
          ...
          cell.t = "n";
          vv = String((cell.v = datenum(cell.v)));
    ...
    switch (cell.t) {
      ...
      default:
        if (opts.bookSST) {
          v = writetag("v", String(get_sst_id(opts.Strings, cell.v)));
          o.t = "s";
          break;
        }
        o.t = "str";
...
*/
  ...
  function parse_xlml_data(xml, ss, data, cell, base, styles, csty, row, o) {
    ...
    switch (data.Type) {
      ...
      /* falls through */
      case "Number":
        if (cell.v === undefined) cell.v = Number(xml);
        if (!cell.t) cell.t = "n";
        break;
      case "Error":
        cell.t = "e";
      ...
      switch (Rn[3]) {
        ...
        case "Cell":
          if (Rn[1] === "/") {
            ...
            ++c;
            if (cell.MergeAcross) c += Number(cell.MergeAcross);
          } else {
            cell = xlml_parsexmltagobj(Rn[0]);
            if (cell.Index) c = Number(cell.Index) - 1;
        ...
        case "Row":
          if (Rn[1] === "/" || Rn[0].substr(-2) === "/>") {
            ...
            if (Rn[0].substr(-2) === "/>") {
              ...
              if (row.Index) r = Number(row.Index) - 1;
            }
            c = 0;
            ++r;
          } else {
        ...
        case "Column":
          ...
          for (var i = 0; i < Number(csty.Span); ++i)
  ...
  function sheet_to_csv(sheet, opts) {
    ...
    for (R = r.s.r; R <= r.e.r; ++R) {
      ...
      for (C = r.s.c; C <= r.e.c; ++C) {
        val = sheet[cols[C] + rr];
        txt = val !== undefined ? String(format_cell(val)) : "";
